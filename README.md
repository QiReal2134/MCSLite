...
面板背景默认是纯色底(`#0d0f14`)加一层很淡的黑到灰渐变,不加载任何图片,首屏几乎是瞬时的。
要换成自己的图,在「设置 → 外观与背景」上传,或选用面板目录内已有的图片;
也可填 `web/bg.jpg` 用回附带的示例图(约 3MB,会明显变慢)。
...
- 世界存档与备份的下载默认需要登录(存档含玩家数据,打包时服务端会暂停保存);
  要让玩家端自助下载,在「设置 → 玩家端下载」中开启,每 IP 限速仍然生效
...
- 备份/存档下载报「未找到 zip 命令」(Linux):`apt install zip`
- 升级到本版本后玩家端不再显示「存档下载」:默认改为需登录,
  可在「设置 → 外观与背景 → 玩家端下载」中恢复为公开

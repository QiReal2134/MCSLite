...
// level-name 存在 server.properties 里且可被改写,必须先确认它解析后仍在实例目录内,
// 否则备份/实时存档会打包到实例之外(如 level-name=.. 会把整个 servers/ 打进去)
function worldFolders(instanceDir, levelName) {
  const base = path.resolve(instanceDir);
  const name = String(levelName || 'world').replace(/\\/g, '/').replace(/^\/+|\/+$/g, '');
  const parts = name.split('/');
  if (!name || parts.some(s => !s || s === '.' || s === '..')) return [];
  const inside = (rel) => {
    const p = path.resolve(base, rel);
    return (p === base || p.startsWith(base + path.sep)) ? p : null;
  };
  const candidates = [];
  for (const rel of [name, name + '_nether', name + '_the_end']) {
    const p = inside(rel);
    if (p && fs.existsSync(p)) candidates.push(rel);
  }
  return candidates;
}

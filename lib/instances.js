...
class InstanceManager {
  constructor({ root, dataDir, hub, getConfig }) {
    ...
    this._packs = 0;                 // 全局打包中任务数(备份 + 实时存档共享上限)
  ...
  load() {
    ...
    // 收养上次面板退出时仍在运行的进程
    for (const ins of this.instances) {
      if (ins.pid && this.isAlive(ins.pid) && this.pidLooksLikeJava(ins.pid) !== false) {
        ins._adopted = true;
        this.appendLog(ins.id, `[面板] 检测到服务端进程仍在运行 (PID ${ins.pid}),已接管监控`);
      } else {
        const reused = ins.pid && this.isAlive(ins.pid);
        ins.pid = null; ins.startedAt = 0;
        if (reused) {
          this.appendLog(ins.id, '[面板] 记录的进程号已被其他程序占用,不接管(避免误杀无关进程)');
        }
      }
    }
    this.save();
  ...
  // 已退出但未被回收的僵尸进程对 kill(pid,0) 仍算存活(Linux 下收养的实例不是我们的子进程,
  // 收不了尸),会让 stop 白等满超时。这里额外查 /proc 状态
  isZombie(pid) {
    if (process.platform !== 'linux' || !pid) return false;
    try {
      const stat = fs.readFileSync(`/proc/${pid}/stat`, 'utf8');
      const end = stat.lastIndexOf(')');
      return end > 0 && stat.slice(end + 1).trim().startsWith('Z');
    } catch { return false; }
  }

  // 等进程退出时使用:僵尸视为已退出
  procAlive(pid) { return this.isAlive(pid) && !this.isZombie(pid); }
  ...
  // 认领上次运行遗留的 pid 前先确认它确实是 Java:进程号会被系统回收,
  // 面板重启/机器重启后同号进程可能是别的东西,认错会让 stop/kill 打到无关进程上。
  // 返回 true=是 Java,false=明确不是,null=无法判断(此时沿用旧的信任策略)
  pidLooksLikeJava(pid) {
    if (!pid) return null;
    try {
      if (process.platform === 'linux') {
        const comm = fs.readFileSync(`/proc/${pid}/comm`, 'utf8').trim();
        return /java/i.test(comm);   // JVM 的 comm 为 java / javaw
      }
  ...
  async startInner(ins) {
    ...
    // spawn 失败(如 java 路径不存在)以异步 error 事件上报,同步 try/catch 接不住;
    // 等 spawn / error 二者之一再往下走,避免「根本没起来却回报成功」
    try {
      await new Promise((resolve, reject) => {
        const onSpawn = () => { child.off('error', onError); resolve(); };
        const onError = (e) => { child.off('spawn', onSpawn); reject(e); };
        child.once('spawn', onSpawn);
        child.once('error', onError);
      });
    } catch (e) {
      const msg = (e && e.message) || String(e);
      this.appendLog(ins.id, '启动失败: ' + msg);
    ...
    // 子进程退出后仍可能有待写入(如 tps 探测),EPIPE 以异步 error 事件抛出,write 外层的 try/catch 接不住
    ...
    child.on('exit', (code) => {
      ...
      this.hub.send(EV.STATUS, { id: ins.id, status: 'stopped' });
      const ws = this.logStreams.get(ins.id);
      if (ws) { try { ws.end(); } catch {} this.logStreams.delete(ins.id); }
      // 进程退出后立即探测一次(玩家页面及时刷新)
      this.probeOnce(ins);
    });

    this.hub.send(EV.STATUS, { id: ins.id, status: 'running', pid: child.pid });
  ...
  async stop(id) {
    ...
    if (ins._childStdin && ins._childStdin.writable) {
      ...
      // 接管的外部进程(面板重启后收养的),没有控制台管道。Linux 先发 SIGTERM 让 JVM 跑关服钩子
      // 保存世界,超时未退再强杀;Windows 没有等价的温和信号,只能强杀
      this.appendLog(ins.id, '[面板] 该进程非本面板启动,无法发送 stop,改用终止信号');
      if (process.platform !== 'win32') {
        try { process.kill(pid, 'SIGTERM'); } catch {}
        const deadline = Date.now() + STOP_TIMEOUT;
        while (Date.now() < deadline && this.procAlive(pid)) await sleep(500);
      }
      if (this.procAlive(pid)) {
        this.appendLog(ins.id, '[面板] 进程未响应终止信号,强制结束');
  ...
  async restart(id) {
    ...
    const ins = this.get(id);
    if (ins) {
      for (let i = 0; i < 20 && this.procAlive(ins.pid); i++) await sleep(300);
    }
  ...
  async createBackup(id, { isAuto = false, name = '' } = {}) {
    ...
    try {
      ...
      if (isAuto) return null;  // 全局并发已满时自动备份也退让,下个周期再来
      throw e;
    }
  ...
  // 打包(备份 / 实时存档)全局并发闸:压缩是 CPU + 磁盘密集操作,
  // 同时开多个会把在线服务端拖慢,也会把磁盘打满。超限直接拒绝,让调用方稍后重试
  acquirePack() {
    const MAX_PACKS = 2;
    if (this._packs >= MAX_PACKS) {
      throw Object.assign(new Error('已有打包任务在进行,请稍后再试'), { status: 429 });
    }
    this._packs++;
  }

  releasePack() {
    if (this._packs > 0) this._packs--;
  }

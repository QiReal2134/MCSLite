...
class InstanceManager {
  ...
  appendLog(id, line, stamp = true) {
    ...
    if (!clean) return;
    // TPS 响应捕获(发送 tps 命令后匹配输出)
    if (this._tpsPending.has(id)) {
      const m = /([\d.]+)\s*[,\s]+\s*([\d.]+)\s*[,\s]+\s*([\d.]+)/.exec(clean);
      if (m) {
  ...
  readProps(ins) {
    ...
    const jars = findServerJars(dirPath);
    return { props: parsed.map, eula, jars: jars.map(j => j.path), text };
  }
  ...
  // ---------- 探测 ----------
  pingTarget(ins) {
    let host = ins.serverIp || this.readProps(ins).props['server-ip'] || '';
    const port = parseInt(this.readProps(ins).props['server-port'] || '25565', 10) || 25565;
